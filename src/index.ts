...
import { issuer, didRegisterAndImport, issuerId, didKey } from "./issuer";
import {
  utils,
  CredentialState,
...
const schemaAndCredentialDefinitionCreation = async () => {
  try {
    if (!issuerId) {
      await didRegisterAndImport(issuer); // Garante que issuerId é inicializado
    }
...
const connectionEstablishment = async () => {
  ...
  console.log("------------>", issuerConnectionRecord[0].id );
